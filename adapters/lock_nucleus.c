...
LOCK_HANDLE Lock_Init(void)
{
    ...
    NU_SPINLOCK* spinlock_ptr = malloc(sizeof(NU_SPINLOCK));
    if (spinlock_ptr == NULL)
    {
    	printf("Spin Lock creation faild.\n");
    }
    else
    {
    	status = NU_Spinlock_Create(spinlock_ptr,0);
    }

    if(status== NU_SUCCESS)
    {
    	return (void*)spinlock_ptr;
    }
    else
    {
    	return 0;
...
LOCK_RESULT Lock(LOCK_HANDLE handle)
{
    ...
    else 
    {
		...
		take_a_lock(handle);*/
    	status = NU_Spinlock_Try((NU_SPINLOCK*)handle);
...
LOCK_RESULT Unlock(LOCK_HANDLE handle)
{
    ...
    else
    {
    	...
    	status = NU_Spinlock_Release((NU_SPINLOCK*)handle);
    	if(status != NU_SUCCESS)
    	{
    		result = LOCK_ERROR;
    	}
...
LOCK_RESULT Lock_Deinit(LOCK_HANDLE handle)
{
    ...
    else
    {
    	...
    	/* Delete the spinlock */
    	status = NU_Spinlock_Delete((NU_SPINLOCK*)handle);
